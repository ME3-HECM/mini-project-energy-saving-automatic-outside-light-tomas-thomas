...
// page 203 in datasheet
// We are using PIR2 because for bits 0, 1, 2 we can use a comparator x interrupt flag bit 
...
// Comparator Setup
// C1Ix
    PIE2bits.C1IE = 1; // E for enable: enables interrupt source  comparator
    PIR2bits.C1IF = 0; // F for flag: if 0 comparator output hasn't changed, if 1 has been changed - reset with software
    IPR2bits.C1IP = 1; // P for priority: set 1 to high priority
...
{
// LED WAVE
	if(PIR2bits.C1IF){ 		//check the interrupt source - if the PIR2bits.C1IF (flag) goes to 1 then execute the code
